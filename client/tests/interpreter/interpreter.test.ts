...
describe("Display order of inherited parameters", () => {
  /**
  ...
  const testTemplate: TemplateInterface = {
    modelicaPath: testTemplatePath,
    name: "Test Template",
    systemTypes: [],
    pathModifiers: {},
  ...
  const testConfig = {
    id: "test-template-config",
    name: "Test Template Config",
    templatePath: testTemplatePath,
    selections: {},
  } as ConfigInterface;

  // Depth-first flattening of the display tree: Dialog tabs and groups do
  // not change the relative display order of the parameters they contain
  const flattenDisplayOptions = (
    items: (FlatConfigOptionGroup | FlatConfigOption)[],
  ): FlatConfigOption[] =>
    items.flatMap((item) =>
      "groupName" in item
        ? flattenDisplayOptions(item.items)
        : [item as FlatConfigOption],
    );

  it("Displays unmodified base class parameters before parameters declared in derived classes", () => {
    const context = new ConfigContext(
      testTemplate,
      testConfig,
      testTemplateOptions,
    ...
    const rootGroup = _formatDisplayGroup(
      context.getRootOption(),
      context.getRootInstance() as OptionInstance,
      context,
    );

    expect(rootGroup).toBeTruthy();

    // Flattened display options, in display order
    const flatItems = flattenDisplayOptions(rootGroup!.items);
    const itemPaths = flatItems.map((item) => item.modelicaPath);

    // Parameter declared in the base class ExtendInterface and never modified
    // in the derived class TestTemplate
    const inheritedIndex = itemPaths.indexOf(
      "TestPackage.Interface.ExtendInterface.interface_unmodified_bool",
    );
    // Parameter declared in the base class NestedExtendInterface (deepest in
    // the inheritance tree) and never modified in derived classes
    const nestedInheritedIndex = itemPaths.indexOf(
      "TestPackage.Interface.NestedExtendInterface.nested_interface_unmodified_bool",
    );
    // Parameter declared in the template itself
    const declaredIndex = itemPaths.indexOf(
      "TestPackage.Template.TestTemplate.nullable_bool",
    );

    expect(inheritedIndex).toBeGreaterThan(-1);
    expect(nestedInheritedIndex).toBeGreaterThan(-1);
    expect(declaredIndex).toBeGreaterThan(-1);
    // Modelica tools display parameters inherited from base classes first,
    // starting with the deepest class in the inheritance tree
    expect(nestedInheritedIndex).toBeLessThan(inheritedIndex);
    expect(inheritedIndex).toBeLessThan(declaredIndex);
  });

  it("Displays modified base class parameters at their declaration position with the modified value", () => {
    const context = new ConfigContext(
      testTemplate,
      testConfig,
      testTemplateOptions,
    ...
    // interface_modified_bool is declared in the base class ExtendInterface
    // just before interface_unmodified_bool, and assigned false in the extends
    // clause of TestTemplate: the modification must not change its display
    // position, which follows the declaration order in ExtendInterface
    const modifiedIndex = itemPaths.indexOf(
      "TestPackage.Interface.ExtendInterface.interface_modified_bool",
    );
    const unmodifiedIndex = itemPaths.indexOf(
      "TestPackage.Interface.ExtendInterface.interface_unmodified_bool",
    );

    expect(modifiedIndex).toBeGreaterThan(-1);
    expect(modifiedIndex).toBe(unmodifiedIndex - 1);

    // The displayed value is the one assigned in the extends clause of
    // TestTemplate, not the declaration value in ExtendInterface
    expect(flatItems[modifiedIndex].value).toBe("false");
    expect(flatItems[unmodifiedIndex].value).toBe("true");

    // interface_param is also modified in the extends clause of TestTemplate
    // but is not displayed (String parameters are filtered out from the
    // configuration panel): only check its evaluated value
    expect(
      itemPaths.indexOf(
        "TestPackage.Interface.ExtendInterface.interface_param",
      ),
    ).toBe(-1);
    expect(resolveToValue("interface_param", context)).toBe("Updated Value");
  });
});

describe("Display of Dialog group and tab annotations", () => {
  /**
   * Options generated from server/tests/static-data/TestPackage via server parser.
   * This file is generated by running:
   *   cd server && npx jest tests/integration/parser/modifiers.test.ts
  ...
  const isGroup = (
    item: FlatConfigOptionGroup | FlatConfigOption,
  ): item is FlatConfigOptionGroup => "groupName" in item;
  ...
  it("Merges Dialog groups with the same name across the inheritance hierarchy", () => {
    const rootGroup = getRootGroup();

    const configurationGroups = rootGroup.items.filter(
      (item) => isGroup(item) && item.groupName === "Configuration",
    ) as FlatConfigOptionGroup[];
    // A single group is created for all parameters annotated with
    // Dialog(group="Configuration"), wherever they are declared in the
    // inheritance hierarchy
    expect(configurationGroups.length).toBe(1);

    const groupPaths = configurationGroups[0].items
      .filter((item) => !isGroup(item))
      .map((item) => (item as FlatConfigOption).modelicaPath);

    // Parameter declared in NestedExtendInterface (deepest base class)
    const nestedInheritedIndex = groupPaths.indexOf(
      "TestPackage.Interface.NestedExtendInterface.nested_interface_unmodified_bool",
    );
    // Parameter declared in ExtendInterface
    const inheritedIndex = groupPaths.indexOf(
      "TestPackage.Interface.ExtendInterface.interface_unmodified_bool",
    );
    // Parameter declared in the template itself
    const declaredIndex = groupPaths.indexOf(
    ...
    expect(inheritedIndex).toBeGreaterThan(-1);
    expect(declaredIndex).toBeGreaterThan(-1);
    // Within the merged group, parameters keep the inheritance display
    // order: deepest base class first, derived classes last
    expect(nestedInheritedIndex).toBeLessThan(inheritedIndex);
    expect(inheritedIndex).toBeLessThan(declaredIndex);
  });

  it("Renders Dialog tabs as groups enclosing their Dialog groups", () => {
    const rootGroup = getRootGroup();

    // nullable_bool is annotated with Dialog(group="Groupy", tab="Tabby")
    const tabbyGroup = rootGroup.items.find(
    ...
    const nullableBool = groupyGroup.items.find(
      (item) =>
        !isGroup(item) &&
        item.modelicaPath === "TestPackage.Template.TestTemplate.nullable_bool",
    );
    expect(nullableBool).toBeTruthy();

    // The parameter is only displayed inside its tab and group
    const topLevelPaths = rootGroup.items
      .filter((item) => !isGroup(item))
      .map((item) => (item as FlatConfigOption).modelicaPath);
    expect(
      topLevelPaths.indexOf("TestPackage.Template.TestTemplate.nullable_bool"),
    ).toBe(-1);
  });

  it("Displays parameters without Dialog annotations headerless at the top", () => {
    const rootGroup = getRootGroup();

    // expression_bool has no Dialog annotation: it is displayed at the top
    // level, before any Dialog group
    const ungroupedIndex = rootGroup.items.findIndex(
      (item) =>
        !isGroup(item) &&
        item.modelicaPath ===
          "TestPackage.Template.TestTemplate.expression_bool",
    );
    const firstDialogGroupIndex = rootGroup.items.findIndex(
      (item) => isGroup(item) && item.groupName === "Configuration",
    );

    expect(ungroupedIndex).toBeGreaterThan(-1);
    expect(firstDialogGroupIndex).toBeGreaterThan(-1);
    expect(ungroupedIndex).toBeLessThan(firstDialogGroupIndex);
  });

  it("Keeps a replaceable component's parameters in their own group nested inside its Dialog group", () => {
    const rootGroup = getRootGroup();

    // selectable_component_with_relative_paths is annotated with
    // Dialog(group="Selectable Component")
    const dialogGroup = rootGroup.items.find(
      (item) => isGroup(item) && item.groupName === "Selectable Component",
    ) as FlatConfigOptionGroup;
    expect(dialogGroup).toBeTruthy();

    // The component selection input is displayed inside the Dialog group
    const componentOption = dialogGroup.items.find(
      (item) =>
        !isGroup(item) &&
        item.modelicaPath ===
          "TestPackage.Template.TestTemplate.selectable_component_with_relative_paths",
    );
    expect(componentOption).toBeTruthy();

    // The component parameters keep their own group ("Replaceable Component"
    // is the component description), nested inside the Dialog group
    const componentGroup = dialogGroup.items.find(
      (item) => isGroup(item) && item.groupName === "Replaceable Component",
    ) as FlatConfigOptionGroup;
    expect(componentGroup).toBeTruthy();
    expect(componentGroup.items.length).toBeGreaterThan(0);
  });

  it("Does not merge a Dialog group into a composition group with a matching name", () => {
    const rootGroup = getRootGroup();

    // collision_bool is annotated with Dialog(group="Replaceable Component"),
    // which matches the description of the composition groups created for
    // the selectable_component* components: the Dialog group must be created
    // as a separate top-level group, not merged into a composition group
    const collisionGroups = rootGroup.items.filter(
      (item) => isGroup(item) && item.groupName === "Replaceable Component",
    ) as FlatConfigOptionGroup[];
    expect(collisionGroups.length).toBeGreaterThan(1);

    const dialogGroups = collisionGroups.filter((group) =>
      group.items.some(
        (item) =>
          !isGroup(item) &&
          item.modelicaPath ===
            "TestPackage.Template.TestTemplate.collision_bool",
      ),
    );
    expect(dialogGroups.length).toBe(1);

    // The composition groups keep the component parameters, e.g.
    // is_another_param declared in SecondComponent
    const compositionGroups = collisionGroups.filter((group) =>
      group.items.some(
        (item) =>
          !isGroup(item) &&
          item.modelicaPath ===
            "TestPackage.Component.SecondComponent.is_another_param",
      ),
    );
  ...
  it("Registers a Dialog group at the declaration position of a parameter hidden by a final assignment", () => {
    ...
    // nested_final_bool is declared in NestedExtendInterface (deepest base
    // class) with Dialog(group="Final Group") before any parameter of the
    // "Configuration" group, and hidden by a final assignment in the extends
    // clause of TestTemplate. The "Final Group" position must follow the
    // declaration of nested_final_bool: before the "Configuration" group,
    // although the only displayed member of the group
    // (final_group_member_bool) is declared last in TestTemplate
    const finalGroupIndex = rootGroup.items.findIndex(
      (item) => isGroup(item) && item.groupName === "Final Group",
    );
    const configurationGroupIndex = rootGroup.items.findIndex(
      (item) => isGroup(item) && item.groupName === "Configuration",
    );

    expect(finalGroupIndex).toBeGreaterThan(-1);
    expect(configurationGroupIndex).toBeGreaterThan(-1);
    expect(finalGroupIndex).toBeLessThan(configurationGroupIndex);

    // The group only contains the displayed member: the parameter hidden by
    // the final assignment is not displayed
    const finalGroup = rootGroup.items[
      finalGroupIndex
    ] as FlatConfigOptionGroup;
    const groupPaths = finalGroup.items
      .filter((item) => !isGroup(item))
      .map((item) => (item as FlatConfigOption).modelicaPath);
  ...
  it("Does not display a Dialog group whose parameters are all hidden", () => {
    const rootGroup = getRootGroup();

    // nested_hidden_group_bool is the only parameter annotated with
    // Dialog(group="Hidden Group") and is hidden by a final assignment in
    // the extends clause of TestTemplate: the group must not be displayed
    const findGroup = (
      items: (FlatConfigOptionGroup | FlatConfigOption)[],
      groupName: string,
    ): FlatConfigOptionGroup | undefined => {
      for (const item of items) {
        ...
        const nested = findGroup(item.items, groupName);
        if (nested) {
          return nested;
        }
      }
    ...
    expect(findGroup(rootGroup.items, "Hidden Group")).toBeUndefined();
  });
});
