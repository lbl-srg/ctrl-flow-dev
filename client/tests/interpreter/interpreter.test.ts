...
describe("Display order of inherited parameters", () => {
   ...
   */
  const testTemplateOptions: { [key: string]: OptionInterface } =
  ...
  it("Displays unmodified base class parameters before parameters declared in derived classes", () => {
    ...
    // Direct (non-group) items, in display order
    const directItems = rootGroup!.items.filter(
      (item) => "modelicaPath" in item,
    ) as FlatConfigOption[];
    const itemPaths = directItems.map((item) => item.modelicaPath);
  ...
  it("Displays modified base class parameters at their declaration position with the modified value", () => {
    ...
    expect(directItems[modifiedIndex].value).toBe("false");
    expect(directItems[unmodifiedIndex].value).toBe("true");
    ...
    expect(
      itemPaths.indexOf("TestPackage.Interface.ExtendInterface.interface_param"),
    ).toBe(-1);
